...
exports.createConsultation = catchAsync(async (req, res) => {
  ...
  if (!user) {
    ...
    updatedUser = await TempUser.findByIdAndUpdate(
      user._id,
      {
        consultations: user.consultations,
        NoOfConsultations: user.NoOfConsultations,
      },
      { new: true }
    );
...
exports.getConsultations = catchAsync(async (req, res) => {
  ...
  try {
    const consultations = await Consultation.find({ contact }).sort(
      "-createdAt"
    );
...
exports.uploadPrescription = async (req, res) => {
  try {
    ...
    upload.array("prescriptions", 5)(req, res, async (err) => {
      // Allow up to 5 files
      if (err) {
      ...
      consultation.prescription = {
        ...consultation.prescription,
        files: [...(consultation.prescription?.files || []), ...fileUrls],
      };
